...
public class RefinementController {
	...
	public ResponseEntity<TopologyTemplate> refineTopologyTemplate(@PathVariable("id") long idTopologyTemplate, @PathVariable("smi") float smi, @RequestBody LevelGraph levelGraph) {
		...
		// Determine the depth of the LevelGraph. ATTENTION the depth is in this case not equal the number of different levels of the Graph because it may be a merged LevelGraph
		...
		for (TopologyTemplate tempTopology : topologyTemplate.getChildTopologyTemplates()) {
			topologyTemplateService.update(tempTopology);
		}
